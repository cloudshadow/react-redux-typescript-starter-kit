...
function generateContainer(upperCaseName,lowerCaseName) {
  ...
  const targetList = [
    {targetText:`<Switch>`, insertText:`            <Route path="/${lowerCaseName}" component={${upperCaseName}Page} />`, adjustLine: +2},
    {targetText:`const store = configureStore();`, insertText:`import ${upperCaseName}Page from './containers/${upperCaseName}Page';`, adjustLine: -2}
  ]
...
function generateAction(upperCaseName,lowerCaseName) {
  ...
  createFile(upperCaseName,lowerCaseName,filePath,tempFilePath);
  const targetList = [
    {targetText:`const rootAction = {`, insertText:`import { ${lowerCaseName}Actions } from './${lowerCaseName}Actions';`, adjustLine: -2},
...
function generateEpic(upperCaseName,lowerCaseName) {
  ...
  createFile(upperCaseName,lowerCaseName,filePath,tempFilePath);
  const targetList = [
    {targetText:`const rootEpic = combineEpics(`, insertText:`import { get${upperCaseName}ResponseEpic } from './${lowerCaseName}Epics';`, adjustLine: -2},
...
function generateReducer(upperCaseName,lowerCaseName) {
  ...
  createFile(upperCaseName,lowerCaseName,filePath,tempFilePath);
  const targetList = [
    {targetText:`export interface IAppState {`, insertText:`import ${lowerCaseName}State, { I${upperCaseName}State } from './${lowerCaseName}Reducer';`, adjustLine: -2},
    {targetText:`export interface IAppState {`, insertText:`  ${lowerCaseName}State: I${upperCaseName}State;`, adjustLine: +1},
...
function createFile(upperCaseName,lowerCaseName,filePath,tempFilePath) {
  ...
  fs.readFile(tempFilePath, 'utf8', function (err,data) {
    ...
    if( type === 'js' ){
      ...
      result = data.replace(/Template/g, upperCaseName).replace(/template/g, lowerCaseName).replace(/TEMPLATE/g, upperCaseName.toUpperCase());
    }
